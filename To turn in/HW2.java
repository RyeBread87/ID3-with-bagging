...
public class BuildAndTestDecisionTree
{
  ...
  //RAS - Adding for HW2
	String myDirectoryPath = args[2];
	String fileName;
	//String parts[];
	//String delimiter = "\\\\";
	String argsToPass[] = new String[2];
	String tuneSetFile = args[0];
    ...
    //RAS - Instead of incrementing a counter, I simply loop through every file in the specified directory
    if (directoryListing != null) {
      for (File child : directoryListing) {
    	  fileName = child.getPath();
    	  //parts = fileName.split(delimiter);
    	  //fileName = parts[1];
    	  //System.err.println(fileName);
    	  argsToPass[0] = fileName;
    	  argsToPass[1] = args[1];
    	  Train(fileName, trees);
      }
    }
    
    ListOfExamples tuneSet = GetList(tuneSetFile);
    ListOfExamples testSet = GetList(testSetFile);
    
    int numberOfTrees = trees.size();
    int numberOfTuneExamples = tuneSet.size();
    int numberOfTestExamples = testSet.size();
    TreeNode tree;
    String firstLabelValue = tuneSet.outputLabel.firstValue;
    String secondLabelValue = tuneSet.outputLabel.secondValue;
    String[][] tunePredictions = new String[numberOfTrees][numberOfTuneExamples];
    String[][] testPredictions = new String[numberOfTrees][numberOfTestExamples];
    
	Example[] tuneExampleList = new Example[numberOfTuneExamples];
    Example[] testExampleList = new Example[numberOfTestExamples];
    
    //RAS - loop through all trees and build 2-dimensional array of predictions for each tree/example combination
    for (i = 0; i < numberOfTrees; i++) {
    
    	tree = trees.get(i);
    	
        Test(tunePredictions, tuneSet, tree, i, tuneExampleList);
        
        Test(testPredictions, testSet, tree, i, testExampleList);
    }
    
    //RAS - output "L + | + accuracy" for each value of L in the definition of the combination rule specified in the homework assignment
    //These are used to plot values for discussion
    System.out.println("Tune:");
    Plot(tunePredictions, numberOfTrees, numberOfTuneExamples, firstLabelValue, secondLabelValue, tuneExampleList);
    
    System.out.println("Test:");
    Plot(testPredictions, numberOfTrees, numberOfTestExamples, firstLabelValue, secondLabelValue, testExampleList);
    
    Utilities.waitHere("Hit <enter> when ready to exit.");
  }

  //RAS - this method calculates the accuracy of our ensemble against a list of examples using the provided definition of the combination rule
  //for odd values of L from 1 to 101
private static void Plot(String[][] predictions, int rows, int columns, String firstValue, String secondValue, Example[] exampleList) {
	...
	int numberCorrect = 0;
	double accuracy = 0;
	Hashtable<Integer, Double> accuracyList = new Hashtable<Integer, Double>();
	String[] ensemblePredictions = new String[columns];
	String ensemblePrediction;
	String actualExampleLabelValue;
	Example example;
	...
	for (L = 1; L < 102; L = L + 2) {
		...
		for (c = 0; c < columns; c++) {
			...
			for (r = 0; r < rows; r++) {
				...
				if (firstValue.equals(predictions[r][c])) {
					firstValueCount++;
				}
			}
			
			if (firstValueCount >= L) {
				ensemblePredictions[c] = firstValue;
			}
		
			else {
				ensemblePredictions[c] = secondValue;
			}
			
			example = exampleList[c];
			actualExampleLabelValue = example.label;
			ensemblePrediction = ensemblePredictions[c];		//compare the ensemble prediction to the example's value
			
			if (ensemblePrediction.equals(actualExampleLabelValue)) {
				numberCorrect++;
			}
		}
		
		accuracy = 100 * ((double) numberCorrect / columns);
		
		accuracyList.put(L, accuracy);
		
		//System.out.println("The accuracy for " + Integer.toString(L) + " is " + String.valueOf(accuracy) + "% (" + String.valueOf(numberCorrect) + " / " + String.valueOf(columns) + ")");
		
		System.out.println(Integer.toString(L) + "|" + String.valueOf(accuracy));
		
		//System.out.println();
	}
  }
  
  private static ListOfExamples GetList(String list) {

	    ListOfExamples examples = new ListOfExamples();
	    if (!examples.ReadInExamplesFromFile(list))
	    {
...
//RAS - Split off training portion.
  public static void Train(String trainSetFile, ArrayList<TreeNode> trees) {
	    // Read in the examples from the files.
	    ListOfExamples trainExamples = GetList(trainSetFile);
  ...
  public static void Test(String[][] predictionArray, ListOfExamples testExamples, TreeNode root, int treeNumber, Example[] exampleList) {
	  
      //testExamples.DescribeDataset();
      //testExamples.PrintThisExample(0);  // Print out an example
      //testExamples.PrintAllExamples(); // Don't waste paper printing all 
                                          // of this out!
      
      //double totalEntropy = totalEntropy(testExamples);
	    ...
	    while (enumeration.hasMoreElements()) {
	    	...
	    	predictionArray[treeNumber][count] = predictedLabel;
	    	
	    	if ((exampleList == null) || (exampleList[count] == null)) {
		    	exampleList[count] = testExample;
	    	}
